(function(w){
	...
	//竖向滑屏
 	w.charles.vMove = function(wrap){
 		
 		var list = wrap.children[0];
 		//3D加速
 		charles.css(list,"translateZ",1);
 		//滑屏的最大值
 		var max = wrap.clientHeight - list.offsetHeight;
 		//手指和元素的起始位置；
 		var startP = {};
 		var elementP = {};
 		//上一次时间和位置
		...
		var Tween={
 		...
 		wrap.addEventListener("touchmove",function(ev){
 			...
 			if(translateY>0){
 				...
 				translateY = charles.css(list,"translateY") + pointV*scale;
 				list.elasticd = true;
 			}else if(translateY<max){
 				var over = max - translateY;
